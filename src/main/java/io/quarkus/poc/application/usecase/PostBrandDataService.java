...
@Slf4j
public class PostBrandDataService implements PostTransactionUseCase {
    ...
    public PostBrandDataService(UnitOfWork unitOfWork, EventPublisherPort eventPublisher, InvoiceGroupRepository invoiceGroupRepository, InvoiceGroupAggregateRootLoader loader, List<EventHandler> eventHandlers) {
    ...
    private InvoiceGroupAggregateRoot runInTransaction(PostingTransactionCommand command) {
        log.info("Processing brand data for command {}", command);
