...
@Slf4j
public class PostReversalTransactionService implements PostTransactionUseCase {
    ...
    public PostReversalTransactionService(UnitOfWork unitOfWork, EventPublisherPort eventPublisher, InvoiceGroupRepository invoiceGroupRepository, InvoiceGroupAggregateRootLoader loader, List<EventHandler> eventHandlers) {
    ...
    private InvoiceGroupAggregateRoot runInTransaction(PostingTransactionCommand command) {
        log.info("Processing reversal transaction for command {}", command);
